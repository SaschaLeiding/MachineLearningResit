...
# (2) Regularized Model - LASSO - lambda 0.1
{
  ...
  # Plot most important words
  {
    ...
    wordcloud(words = tidy(model_regul_corr)$term,
              freq = tidy(model_regul_corr)$estimate,
              max.words = 100)
  }
}
...
# (3) Regularized Model - LASSO - lambda Cross-validated(CV)
{
    ...
    {
      # Plot in-sample fit
      plot_fit_cvregul_corr_in <- ggplot(data = (train %>% 
                                                   bind_cols(predict(model_cvregul_corr, newx=as.matrix(X_train),
    ...
    # Plot out_of-sample fit
      plot_fit_cvregul_corr_out <- ggplot(data = (test %>% 
                                                   bind_cols(predict(model_cvregul_corr, newx=as.matrix(X_test),
                                                                     s="lambda.min", type="link"))),
                                     aes(x = lambda.min, y = .corrindex)) + 
      geom_point() +
      geom_abline(intercept = 0, slope = 1, size = 0.5) +
...
# partial out W from Z
    fit.lasso.Z <- cv.glmnet(as.matrix(W_train), as.matrix(Z_train))
    print(dim(W_train))
print(dim(Z_train))

# Check for NA values
print(sum(is.na(W_train)))
print(sum(is.na(Z_train)))
    fitted.lasso.Z <- predict(fit.lasso.Z, newx=as.matrix(W_train), s="lambda.min")
    Ztilde <- as.matrix(Z_train) - fitted.lasso.Z
...
# partial out W from Y
    ...
    # partial out W from Z
    fit.lasso.Z <- cv.glmnet(W, Z)
    fitted.lasso.Z <- predict(fit.lasso.Z, newx=W, s="lambda.min")
    Ztilde <- Z - fitted.lasso.Z
