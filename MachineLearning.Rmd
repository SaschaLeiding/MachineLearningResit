...
# Estimation of Treatment Effect
{
  ...
  pppp <- simplemodels_list[[paste0("model_comp_", name_outcome)]] %>% 
    extract_fit_engine() %>%
    summary()
  coef(pppp)
  ...
  # (5) Regularized Model - Tree-based
      # First estimate propensity scores
      #propensity.forest <- regression_forest(X_forest, W)
  {
    test.forest <- causal_forest(as.matrix(W_train), as.matrix(Y_train), 
                                 as.matrix(D_train), tune.parameters = "all")
    
    # Calculate the ATE:
    average_treatment_effect(test.forest)
    ...
    blp <- best_linear_projection(test.forest, as.matrix(W_train))
    blp
  }
  
}

# Plotting of in-/out-of-sample fit for models (1)-(3)
{
