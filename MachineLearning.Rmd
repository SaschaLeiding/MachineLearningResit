...
# Plotting of in-/out-of-sample fit for models (1)-(4) with test and train data
{
  ...
  # Special Case: Double LASSO as rlassoEffects takes too long with that many variables
  {
    ...
    # Step 2: Plot in/-out of sample Fits
    {
      ...
      train$DLpred_corrindex <- fitted.lasso.Y + fit.doubleLASSO$fitted.values
      test$DLpred_corrindex <- fitted.lasso.Y.test + fit.doubleLASSO.test$fitted.values
      
      # Plotting for Double LASSO - In-sample (training set)
      plot_in_sample <- ggplot(train, 
                           aes(x = DLpred_corrindex,
                               y = .corrindex)) +
        geom_point() +
...
{
  pppW <- W_full %>% select(starts_with("."), all_of(words_hetero))
}
# Double LASSO with all units of observation but only selected words
{
  # Partial out W (confounders) from Y (outcome)
    fit.lasso.Yfull <- cv.glmnet(x=as.matrix(pppW), y=as.matrix(Y_full))
    fitted.lasso.Yfull <- predict(fit.lasso.Yfull, newx=as.matrix(pppW), s="lambda.min")
    Ytilde.full <- as.matrix(Y_full) - fitted.lasso.Yfull
    
    # Partial out w(confounders) from D(Party Affiliation)
    fit.lasso.Dfull <- cv.glmnet(x=as.matrix(pppW), y=as.matrix(D_full))
    fitted.lasso.Dfull <- predict(fit.lasso.Dfull, newx=as.matrix(pppW), s="lambda.min")
    Dtilde.full <- as.matrix(D_full) - fitted.lasso.Dfull
    
    # Run OLS of Ytilde on Dtilde
    fit.doubleLASSO.full <- lm(Ytilde.full ~ Dtilde.full)
    beta1hat.full <- coef(fit.doubleLASSO)[2]
    print(beta1hat.full)
}

# Causal Forest
{
  fit.forest <- causal_forest(as.matrix(pppW), as.matrix(Y_full), 
...
# Create interaction term - TO BE ADAPTED
{
  ...
  pppx_interacted <- cbind(D_full, pppW) %>% 
    mutate(across(everything(), ~ . * .partydummy, .names = ".partydummyx{.col}")) %>%
    select(where(~ var(.) != 0)) # drop all columns where there is no variation
}

# Double LASSO
# OLS estimation of residuals is not inherently robust to treatment effect heterogeneity
{
  column_indices <- grep("^\\.party", colnames(pppx_interacted))
  ppptestDL <- rlassoEffects(x=pppx_interacted, y=Y_full, index= column_indices)
}
...
# 2.2 Estimate Effect of Party Affiliation on .corrindex and .logincome
{
  # Double LASSO - CV
  {
    ...
    Dtilde_corr <- as.matrix(D_full) - fitted.lasso.D
