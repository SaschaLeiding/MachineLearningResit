...
simplemodels_list$model_comp_corr <- linear_reg() %>%
  fit_xy(y = Y_train, x = X_train)

# (2) Regularized Model - LASSO
simplemodels_list$model_regul_corr <- linear_reg(penalty = 0.01, mixture = 1) %>%
  set_engine('glmnet') %>%
  fit_xy(y = Y_train, x = X_train)

# (3) Regularized Model - LASSO - lambda Cross-validated (CV)
simplemodels_list$model_cvregul_corr <- cv.glmnet(x = as.matrix(X_train), y = as.matrix(Y_train),
...
# (3) Regularized Model - LASSO - lambda Cross-validated(CV)
{
    model_cvregul_corr <- cv.glmnet(x=as.matrix(X_train), y=as.matrix(Y_train), # Input
                                    nfolds=10, # Number of folds to CV
                                    type.measure = "mse") # the measure used to optimize CV
    plot(model_cvregul_corr) # 1st dot-line gives the min MSE from CV while 2nd gives most regularized model
    print(model_cvregul_corr) #Prints: Lambda, the index of which lambdas (position), the measure value to optimize CV, SE, number of nonzero coefficients
    model_cvregul_corr$lambda.min
        
    # Extract non-zero coefficients at lambda.min
    non_zero_coefficients <- as.data.frame(as.matrix(coef(model_cvregul_corr, s = "lambda.min"))) %>%
      filter(.[[1]] != 0)
    print(non_zero_coefficients)
    
    # Create Wordcloud with non-zero regularized words
    {
      wordcloud(words = rownames(non_zero_coefficients)[!rownames(non_zero_coefficients) %in% '(Intercept)' & 
                                                  !grepl("^\\.", rownames(non_zero_coefficients))],
              freq=non_zero_coefficients[!rownames(non_zero_coefficients) %in% '(Intercept)' & 
                                   !grepl("^\\.", rownames(non_zero_coefficients)), ])
    }
}

# Merge in-/ and out-of-sample fit Plots
{
...
# (4) Regularized Model - Double LASSO
# To-do: DISTINGUISH BETWEEN TRAIN and TEST?
{
  # Partial out confounders from outcome
  model_dlcvregul_corrY <- cv.glmnet(x=as.matrix(W_train), y=as.matrix(Y_train))
  fitted.lasso.Y <- predict(model_dlcvregul_corrY, newx=as.matrix(W_train), s="lambda.min")
  Ytilde_corr <- as.matrix(Y_train) - fitted.lasso.Y
  
  # Partial out confounders from Party Affiliation
  model_dlcvregul_corrD <- cv.glmnet(x=as.matrix(W_train), y=as.matrix(D_train))
  fitted.lasso.D <- predict(model_dlcvregul_corrD, newx=as.matrix(W_train), s="lambda.min")
...
# partial out W from Z
# THIS COMPUTATION TAKES SEVERAL MINUTES
    fit.lasso.Z <- cv.glmnet(x=as.matrix(W_train), y=as.matrix(Z_train), family="multinomial")
    fitted.lasso.Z <- predict(fit.lasso.Z, newx=as.matrix(W_train), s="lambda.min", type ="link")
    Ztilde <- as.matrix(Z_train) - apply(fitted.lasso.Z, 2, c)

    # run IV on residuals
    fit.tripleLASSO.rf <- lm(Ytilde~Ztilde)
    coef.tripleLASSO.rf <- coef(fit.tripleLASSO.rf)[2]		# reduced-form coefficient
    fit.tripleLASSO.fs <- lm(Dtilde~Ztilde)
    coef.tripleLASSO.fs <- coef(fit.tripleLASSO.fs)[2]		# first-stage coefficient
     
    beta1hat.tripleLASSO <- coef.tripleLASSO.rf / coef.tripleLASSO.fs	# IV coefficient
    beta1hat.tripleLASSO
    
  "
  rlassoIV takes more than 30min and could not be finally processed with 16k variables,
  works however when fewer variables are taken.
  testpt <- rlassoIVmult(x=as.matrix(X_train), d=as.matrix(D_train), y=as.matrix(Y_train),
                       z=as.matrix(Z_train), select.Z=FALSE, select.X=TRUE)
  testpt <- rlassoIV(Y_train ~ D_train + W_train | Z_train + W_train, 
...
#Quadruple LASSO IV -because instrument is multinomial
{
  ...
  fit.QDlasso.D <- cv.glmnet(y=as.matrix(D_train), as.matrix((cbind(Z_train, W_train))))
  fitted.QDlasso.D <- predict(fit.QDlasso.D, newx=as.matrix((cbind(Z_train, W_train))), s="lambda.min")
  ...
  fit.QDlasso.QD <- cv.glmnet(as.matrix(W_train), fitted.QDlasso.D)
  fitted.QDlasso.QD <- predict(fit.QDlasso.QD, newx=as.matrix(W_train), s="lambda.min")
  QDtilde <- fitted.QDlasso.D - fitted.QDlasso.QD
  
  # 3. Estimate IV coefficient
  fit.QDLASSO.rf <- lm(Ytilde ~ QDtilde)
  coef.QDLASSO.rf <- coef(fit.QDLASSO.rf)[2]
  fit.QDLASSO.fs <- lm(Dtilde ~ QDtilde)
