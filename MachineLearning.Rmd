  ...
  for (i in seq_along(simplemodels_list)) {
    ...
    if (inherits(model, "cv.glmnet")) {
      ...
      plot_in_sample <- ggplot(data = (train %>% bind_cols(predict_in_sample)),
                               aes(x = lambda.min, y = .logincome, color = .party, group = .party)) +
        geom_point() +
        geom_abline(intercept = 0, slope = 1, size = 0.5) +
        labs(x = "Predicted logincome", y = "Actual logincome") +
      ...
      plot_out_sample <- ggplot(data = (test %>% bind_cols(predict_out_sample)),
        ...
        plot_in_sample <- ggplot(data = (train %>% bind_cols(predict_in_sample)),
                                 aes(x = .pred, y = .logincome, color = .party, group = .party)) +
          geom_point() +
          geom_abline(intercept = 0, slope = 1, size = 0.5) +
          labs(x = "Predicted logincome", y = "Actual logincome") +
          theme(legend.position="none")
    ...
    # Step 2: Plot in/-out of sample Fits
    {
      ...
      plot_in_sample <- ggplot(train, 
                           aes(x = DLpred_logincome,
                               y = .logincome, color = .party, group = .party)) +
        geom_point() +
      geom_abline(intercept = 0, slope = 1, size = 0.5) +
      labs(x = "Predicted logincome", y = "Actual logincome") +
      theme(legend.position="none")
...
pppx <- cbind((D_full %>% rename("partydummy" = ".partydummy")), ZW)

first_stage_model <- lm(formula.fs, data = pppx)
